...
import { ROLE_META, emptyForm } from "../_lib/constants";
...
export default function UsersClient({ initialUsers }: { initialUsers: User[] }) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        const updated = await updateUserAction(editing.id, f);
        setUsers((prev) => prev.map((u) => (u.id === editing.id ? updated : u)));
      } else {
        const created = await createUserAction(f);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Soft-delete (nonaktif) atau hard-delete keduanya menghilangkan baris dari
      // daftar aktif; muat ulang akan menampilkan status terbaru dari DB.
