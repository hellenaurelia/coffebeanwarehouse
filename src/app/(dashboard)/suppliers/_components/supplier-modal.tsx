...
const blankForm: SForm = { name:"", pic:"", region:"", phone:"", email:"", beans:[{ name:"", price:"", type:"" }], address:"", notes:"", status:"Aktif" };
            ...
            <div className="space-y-2">
              {form.beans.map((b, i) => (
                <div key={i} className="flex gap-2 items-center">
                  ...
                  />
                  <select
                    className={`${INP} w-32 bg-secondary/50 border border-input`}
                    value={b.type}
                    onChange={e => {
                      const nb = [...form.beans];
                      nb[i] = { ...nb[i], type: e.target.value };
                      set("beans", nb);
                    }}
                  >
