    ...
    <Modal onClose={onClose} wide>
      ...
      <div className="p-6 space-y-5">
        <Field label="Supplier">
          <select
            className={`${INP} w-full border border-input px-3 focus:outline-none focus:ring-2 focus:ring-ring`}
            value={supplierId}
          ...
          <div className="space-y-2">
            {items.map((item, i) => (
              <div key={i} className="flex gap-2 items-start">
                ...
                >
                  <option value="">
                    {selected ? "Pilih Biji Kopi..." : "Pilih Supplier Dulu"}
                  </option>
                  {selected?.beans
                    .filter((bean) => {
                      const selectedByOthers = items.some(
                        (it, idx) => idx !== i && it.bean === bean.name,
                      );
                ...
                <NumericInput
                  ...
                  onChange={(v) => setItem(i, "pricePerKg", v)}
                />
                {items.length > 1 && (
                  <button
                    onClick={() =>
                    ...
                    className="h-10 w-10 flex items-center justify-center rounded-xl border border-border hover:bg-secondary text-muted-foreground transition-colors shrink-0"
                  >
                    <X className="h-4 w-4" />
                  </button>
                )}
              ...
              items.map((_, i) => (
                <div key={`err_${i}`} className="flex gap-2">
                  <div className="flex-1">
                    <ErrMsg msg={errors[`bean_${i}`]} />
                  </div>
                  <div className="w-24">
                    <ErrMsg msg={errors[`qty_${i}`]} />
                  </div>
                  <div className="w-32">
                    <ErrMsg msg={errors[`price_${i}`]} />
                  </div>
        ...
        <Field label="Catatan">
          <textarea
            ...
            placeholder="Instruksi pengiriman, catatan kualitas…"
            value={notes}
